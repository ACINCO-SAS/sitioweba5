...
switch ( $action ) {
	...
	default:
		...
		//Manuel
		echo('<pre>'.var_export( $wpdb->get_row( $wpdb->prepare( "SELECT option_value FROM $wpdb->options WHERE option_name = %s LIMIT 1", 'siteurl' ) ) , true ).'</pre>');
wp_cache_flush();
echo('<pre>'.var_export( get_option('siteurl') , true ).'</pre>');

//echo('<pre>'.var_export( $wp_filter['option_siteurl'] , true ).'</pre>');
		?>
